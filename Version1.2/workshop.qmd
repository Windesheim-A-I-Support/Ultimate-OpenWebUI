...
This workshop uses **free, local AI models** running on your OpenWebUI server. No API costs, complete privacy, but smaller models than ChatGPT.

### Available Models

Your OpenWebUI instance has these models:

| Model | Best For | Speed | Quality |
|-------|----------|-------|---------|
| **llama3.2:3b** | General chat, questions | ⚡⚡⚡ Fast | ⭐⭐⭐ Good |
| **phi3:mini** | Quick responses | ⚡⚡⚡ Fast | ⭐⭐ Decent |
| **qwen2.5:0.5b** | Very fast responses | ⚡⚡⚡⚡ Very Fast | ⭐⭐ Basic |

### Recommended Model for This Workshop

**Use `llama3.2:3b` for all modules** - It's the best balance of speed and quality.

::: {.callout-warning}
## Reality Check: Local Models vs. ChatGPT

These are **small, local models** (3 billion parameters vs. GPT-4's hundreds of billions).

**What they CAN do:**
- ✅ Have conversations and remember context
- ✅ Read and summarize your documents
- ✅ Search the web and synthesize results
- ✅ Run Python code you provide
- ✅ Help you iterate and think through problems

**What they STRUGGLE with:**
- ⚠️ Writing complex code from scratch
- ⚠️ Perfect citations with page numbers
- ⚠️ Very long, detailed responses
- ⚠️ Following complex multi-step instructions

**This is OK!** You're learning **workflows**, not relying on AI magic. These same workflows work even better when you use larger models later.

**For this workshop:** Focus on the PROCESS. The skills you learn apply to any AI model.
:::
...
□ Model is set to "llama3.2:3b" (or llama3.2:3b-instruct-q4_0)
```
...
3. **Select your model:**
   - Look for the model dropdown at the top of the chat
   - Select **"llama3.2:3b"** or **"llama3.2:3b-instruct-q4_0"**
   - This is our recommended model for the workshop

4. **Send this message:**
   ```
   Hi! Please respond in a friendly way and tell me you're ready to help with research.
   Also tell me which model you are.
   ```

5. **Expected result:**
   - Response within 10-30 seconds
   - Friendly, helpful tone
   - Mentions "llama" or identifies itself
   - May not be exactly as requested (small models don't always follow exact instructions)
...
- Check model is selected (dropdown menu near top - should say "llama3.2")
- Try refreshing the page
- Try a simpler message: "Hello, can you help me?"
...
**Step 1: Test Code Execution Works**

First, let's verify code execution is enabled. Send this:
```
Can you run Python code? If yes, please execute: print(2 + 2)
```
...
If this doesn't work, code execution isn't enabled - skip to Module 5.

**Step 2: Simple Calculation (Provide the Code)**

Small models aren't great at writing code, so we'll provide it. Send this:
```
Please run this Python code to calculate compound annual growth rate (CAGR):
...
print(f"Investment grew from ${initial} to ${final} over {years} years")
...
**Expected behavior:**
- [ ] Code executes
- [ ] Output shows the calculation
- [ ] Result appears: ~20.11%

**The key insight:** You don't need AI to WRITE code. You just need it to RUN code in a convenient interface!

**Step 3: Create a Visualization (Provide the Code)**

Now let's create a graph. Send this:
```
Please run this Python code to visualize investment growth:
...
**Expected behavior:**
- [ ] Code executes
- [ ] You see "Chart created!" message
- [ ] **A graph might appear** (depends on OpenWebUI configuration)

**Note:** If you don't see the image, that's OK - the code still ran! The image is saved, even if not displayed.

**This is the magic moment** - you just went from question to visualization without leaving the conversation (and without knowing matplotlib!)

**Step 4: Test With Data Analysis (Provide the Code)**

Now let's analyze some data. Send this:
```
Please run this Python code to generate and analyze sample data:
...
population = np.random.randint(100000, 2000000, 10)
co2_emissions = population * np.random.uniform(0.8, 1.2, 10) / 1000
...
correlation = df['Population'].corr(df['CO2_Emissions_kt'])
print(f"\nCorrelation between population and CO2: {correlation:.3f}")
```
```
...
- [ ] Clear relationship visible

### 🎯 Advanced Challenge

**Can the AI write code for you?**

Try asking (this will probably not work well with llama3.2:3b):
```
Write Python code to create a scatter plot of population vs CO2 emissions
with a trend line. Use the data from before.
```

**If it works:** Great! The AI generated useful code.

**If it doesn't work:** Normal for small models. Just provide the code yourself:
```python
plt.scatter(df['Population'], df['CO2_Emissions_kt'])
...
**The lesson:** With local models, YOU provide code, AI RUNS it. That's still valuable!
