...
This workshop uses **OpenRouter** to access powerful cloud-based AI models. OpenRouter provides access to GPT-4, Claude, and other leading models through a single interface.

### Available Models via OpenRouter

Your OpenWebUI instance has access to these models:

| Model | Best For | Speed | Quality |
|-------|----------|-------|---------|
| **Claude 3.5 Sonnet** | Complex reasoning, code, citations | ⚡⚡ Good | ⭐⭐⭐⭐⭐ Excellent |
| **GPT-4o** | All-around tasks, fast responses | ⚡⚡⚡ Fast | ⭐⭐⭐⭐⭐ Excellent |
| **GPT-4** | Complex analysis, code generation | ⚡⚡ Good | ⭐⭐⭐⭐⭐ Excellent |
| **Claude 3 Haiku** | Quick tasks, basic queries | ⚡⚡⚡⚡ Very Fast | ⭐⭐⭐⭐ Good |

### Recommended Model for This Workshop

**Use `Claude 3.5 Sonnet` (via OpenRouter) for all modules** - It excels at:
- 📚 Document analysis with accurate citations
- 💻 Python code generation
- 🔗 Synthesizing multiple sources
- 🎯 Following complex instructions

**Alternative:** `GPT-4o` - Slightly faster, also excellent quality

::: {.callout-note}
## About OpenRouter

OpenRouter provides access to multiple AI models through API calls. Your workshop organizer has configured access.

**Benefits:**
- ✅ Access to the most capable AI models available
- ✅ No individual API key setup needed
- ✅ Works seamlessly through OpenWebUI interface
- ✅ High-quality outputs for all workshop tasks

**Note:** This workshop uses cloud models, not local models. Local models (llama3.2, phi3) are available but run on CPU and are too slow for this workshop.
:::
...
2. Look for models with names like:
...
**If you don't see OpenRouter models:** Check with your facilitator - OpenRouter may need to be configured.
:::
...
□ Model is set to "Claude 3.5 Sonnet" (via OpenRouter)
```
...
3. **Select your model:**
   - Look for the model dropdown at the top of the chat interface
   - Find and select **"Claude 3.5 Sonnet"** (it may be listed as "claude-3-5-sonnet" or similar)
   - ⚠️ **IMPORTANT:** Do NOT select local models (llama3.2, phi3, qwen) - they're too slow
   - If you don't see Claude 3.5 Sonnet, select **"GPT-4o"** or **"GPT-4"**
...
5. **Expected result:**
   ...
   - Identifies as Claude, GPT-4, or other cloud model
...
- [ ] Model identifies as Claude or GPT-4 (NOT llama/phi/qwen)

**❌ If it fails:**
- Check you're logged in (see your email in top right?)
- Check model dropdown - should show "Claude" or "GPT-4", NOT "llama3.2"
- If you see a local model name, switch to an OpenRouter model
...
Claude and GPT-4 are excellent at writing code. Let's test it:
```
...
**Look for:** A code block that gets executed, something like:
```python
initial = 100
final = 250
years = 5
...
- [ ] Code executes automatically
- [ ] **A graph appears in the chat** (or you see "Chart created" message)
- [ ] Graph shows growth curve with proper labels

**This is the magic moment** - you went from question to visualization in ONE request, without knowing any matplotlib syntax!

**Note:** If you don't see the image inline, that's OK - it may be saved. The important part is the AI wrote and executed the code.
